...
const getBunnyLibraryId = () => {
  const value = process.env.BUNNY_LIBRARY_ID;
  if (!value) {
    console.warn('BUNNY_LIBRARY_ID not found, using placeholder for development');
    return 'placeholder_library_id';
  }
  return value;
};

const getAccessKeys = () => {
  const streamKey = process.env.BUNNY_STREAM_ACCESS_KEY;
  const storageKey = process.env.BUNNY_STORAGE_ACCESS_KEY;
  
  if (!streamKey || !storageKey) {
    console.warn('Bunny access keys not found, using placeholders for development');
    return {
      streamAccessKey: 'placeholder_stream_key',
      storageAccessKey: 'placeholder_storage_key',
    };
  }
  
  return {
    streamAccessKey: streamKey,
    storageAccessKey: storageKey,
  };
};

const BUNNY_LIBRARY_ID = getBunnyLibraryId();
...
export const getVideoUploadUrl = withErrorHandling(async () => {
  ...
  if (BUNNY_LIBRARY_ID === 'placeholder_library_id' || ACCESS_KEYS.streamAccessKey === 'placeholder_stream_key') {
    console.warn('Using placeholder values for video upload - this will not work in production');
    return {
...
export const getThumbnailUploadUrl = withErrorHandling(
  async (videoId: string) => {
    // Check if we're using placeholder values
    if (ACCESS_KEYS.storageAccessKey === 'placeholder_storage_key') {
      console.warn('Using placeholder values for thumbnail upload - this will not work in production');
      return {
        uploadUrl: 'placeholder_upload_url',
        cdnUrl: 'placeholder_cdn_url',
        accessKey: 'placeholder_access_key',
      };
    }
...
export const saveVideoDetails = withErrorHandling(
  async (videoDetails: VideoDetails) => {
    ...
    // Check if we're using placeholder values
    if (BUNNY_LIBRARY_ID === 'placeholder_library_id' || ACCESS_KEYS.streamAccessKey === 'placeholder_stream_key') {
      console.warn('Using placeholder values for video details - this will not work in production');
